...
/* ---------- AL-state helpers (robust across IgH variants) ---------- */
static const char* al_state_str_fallback(uint8_t s, char buf[16]) {
    snprintf(buf, 16, "0x%02X", s);
    return buf;
}
...
static const char* al_state_str(uint8_t s)
{
    ...
    /* Try known names if they are present in this header set */
    #ifdef EC_AL_STATE_INIT
    if (s == EC_AL_STATE_INIT)   return "INIT";
    #endif
    #ifdef EC_AL_STATE_PREOP
    if (s == EC_AL_STATE_PREOP)  return "PREOP";
    #endif
    #ifdef EC_AL_STATE_SAFEOP
    if (s == EC_AL_STATE_SAFEOP) return "SAFEOP";
    #endif
    #ifdef EC_AL_STATE_OP
    if (s == EC_AL_STATE_OP)     return "OP";
    #endif
    /* Some IgH variants also define EC_AL_STATE_BOOT; guard in case it's missing */
    #ifdef EC_AL_STATE_BOOT
    if (s == EC_AL_STATE_BOOT)   return "BOOT";
    #endif

    return al_state_str_fallback(s, tmp);
}

/* ---------- Wait for OP with optional keepalive writer ---------- */
static int wait_for_op(ec_master_t *master,
                       ec_slave_config_t *sc,
                       ec_domain_t *domain,
                       int timeout_ms,
                       bool keepalive,
                       unsigned ka_lo_dom_off,  /* SM2 KA low byte domain off  */
                       unsigned ka_hi_dom_off)  /* SM2 KA high byte domain off */
{
    ...
    printf("Waiting up to %d ms for the slave to reach OP (RX/PROC/QUEUE/SEND)...\n", timeout_ms);
    ...
    while (elapsed < timeout_ms) {
        ...
        /* optional keepalive write before queue+send */
        if (keepalive && ka_lo_dom_off != (unsigned)~0u && ka_hi_dom_off != (unsigned)~0u) {
            ka++;
            uint8_t *pd = ecrt_domain_data(domain);
            pd[ka_lo_dom_off] = (uint8_t)(ka & 0xFF);
            pd[ka_hi_dom_off] = (uint8_t)((ka >> 8) & 0xFF);
        }
...
{
    if (argc < 2) {
        fprintf(stderr, "Usage: sudo %s ecat_pdo_config.json [--wait-op-ms 7000] [--keepalive]\n", argv[0]);
        return 1;
    }
    ...
    /* Compute SM2 domain offsets (for keepalive, offsets 58/59 in SM2) */
    unsigned sm2_dom_off[SM2_BYTES];
    for (int i=0;i<SM2_BYTES;i++) sm2_dom_off[i] = all_offsets[i];

    /* Derive keepalive byte domain offsets (lo @ 58, hi @ 59) with bounds checks */
    unsigned ka_lo = (58 < SM2_BYTES) ? sm2_dom_off[58] : (unsigned)~0u;
    unsigned ka_hi = (59 < SM2_BYTES) ? sm2_dom_off[59] : (unsigned)~0u;

    if (keepalive && (ka_lo==(unsigned)~0u || ka_hi==(unsigned)~0u)) {
        fprintf(stderr, "WARNING: keepalive requested but SM2[58/59] not available.\n");
    }

    /* 3) Attempt to reach OP; we drive RX/PROC/QUEUE/SEND in the loop,
          and (optionally) write keepalive each cycle to satisfy the slave.   */
    if (wait_for_op(master, sc, domain, wait_op_ms, keepalive, ka_lo, ka_hi) != 0) {
    ...
    while ((ch = getch()) != 'q') {
        ...
        /* keepalive write (optional) */
        if (keepalive && ka_lo!=(unsigned)~0u && ka_hi!=(unsigned)~0u) {
            ka++;
            dom[ka_lo] = (uint8_t)(ka & 0xFF);
            dom[ka_hi] = (uint8_t)((ka >> 8) & 0xFF);
        }

        attron(COLOR_PAIR(2));
        mvprintw(0, 0, "CIFX RE/ECS Live Viewer  |  SM3 fields  |  q: quit   %s",
