...
/*****************************************************************************/
/* Paste your EXACT cstruct output here                                      */
/*****************************************************************************/
...
void timespec_add(struct timespec *t, uint64_t addtime)
{
    t->tv_nsec += addtime;
...
#include "domain_regs.h"
  ...
  /* Configure Slave */
  sc = ecrt_master_slave_config(master, 0, 0,
				VENDOR_ID,
				PRODUCT_CODE);
  if (!sc) {
  ...
  while (run) {
    ...
    uint64_t app_time =
      now.tv_sec * (uint64_t)NSEC_PER_SEC + now.tv_nsec;
